...
//HELPER FUNCTIONS
//returns a safe number or null if invalid
...
//parses a JSON array field that might be sent as a string or an actual array
...
//resolve clerk user id from various possible locations in the request
...
export const createServiceAppointment = async (req,res) => {
    try{
        ...
        console.log('Service Appointment - req.auth:', req.auth);
        console.log('Service Appointment - clerkUserId resolved:', clerkUserId);
        if (!clerkUserId) {
...
export const confirmServicePayment = async (req,res) => {
    ...
    if (!session_id) return res.status(400).json({ success: false, message: "session_id query parameter is required" });
    if (!stripe) return res.status(500).json({ success: false, message: "Stripe not configured on server" });
    let session;
    try{
...
export const getServiceAppointments = async (req,res) => {
    ...
    const appointments = await ServiceAppointment.find(filter).sort({ createdAt: -1 }).populate("serviceId","name image imageUrl imageSmall").skip(skip).limit(limit).lean();
...
export const cancelServiceAppointment = async (req,res) => {
    ...
    await appt.save();
    return res.status(200).json({ success: true, message: "Appointment canceled", data: appt });
        } 
...
export const getServiceAppointmentStats = async (req,res) => {
    try{
        //it will give the stats of service appointments like total appointments, completed, canceled and earning for each service
