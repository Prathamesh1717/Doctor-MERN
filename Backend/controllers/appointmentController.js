...
//function will get user from clerk and return the user details
...
export const createAppointment = async(req,res) => {
    ...
    if(existingBooking){
        return res.status(409).json({
            success:false,
            message: "You already have appointment with this doctor at the selected slot."
        });
    }
...
try{
    ...
    // Fallback: if no appointments found by doctorId, try matching by owner.
    let countFilter = filter;
    if (items.length === 0) {
      const ownerFilter = { owner: doctorId };
      if (mobile) ownerFilter.mobile = mobile;
      if (status) ownerFilter.status = status;
      if (search) ownerFilter.$or = filter.$or;
      items = await Appointment.find(ownerFilter)
        .sort({ date: 1, time: 1 })
        .skip(skip)
        .limit(limit)
        .populate("doctorId", "name specialization owner imageUrl image")
        .lean();
      countFilter = ownerFilter;
    }

    const total = await Appointment.countDocuments(countFilter);
    return res.json({
        success : true,
        appointments: items,
        meta: {page, limit, total, count:items.length}
