...
//Helper function to handle doctor profile updates

//convert time string like "2:30 PM" to total minutes for easier sorting
...
//dedupe and sort schedule slots for each date , filter slot by validating time format
...
//accept the object or string input for schedule and return a deduped and sorted schedule object
...
//normalize the doctor document before sending to client into plain text
...
export async function createDoctor(req, res) {
    ...
    if(req.file?.path){
        const uploadResult = await uploadToCloudinary(req.file.path, "doctors");
        imageUrl = uploadResult?.secure_url || uploadResult?.url || imageUrl;
        imagePublicId = uploadResult?.public_id || imagePublicId;
    ...
    const token = jwt.sign({ id: doc._id.toString(), email: doc.email,role:"doctor" }, secret, { expiresIn: "1h" });
    const out = normalizeDocForClient(doc.toObject());
    delete out.password;

    return res.status(201).json({ success: true, message: "Doctor profile created successfully", doctor: out, token });
...
export async function updateDoctor(req, res) {
  try {
    ...
    //if new image file is uploaded, upload to cloudinary and update the url and public id, also delete the previous image from cloudinary if exists  
    if (req.file?.path) {
  ...
  try{
    ...
    if(!existing){
      ...
      if(existing.imagePublicId){
        deleteFromCloudinary(existing.imagePublicId).catch((e) => console.warn("deleteFromCloudinary warning:", e?.message || e));
      }
    }
...
//to toggle doctor availability status
    ...
    if(!req.doctor || String(req.doctor._id || req.doctor.id) !== String(id)) {
     ...
     if(!doc){
      ...
      if(typeof doc.availability === "boolean"){
        doc.availability = doc.availability ? "Unavailable" : "Available";
      }else{
      ...
      const out = normalizeDocForClient(doc.toObject());
      delete out.password;
      return res.json({ success: true, message: "Doctor availability toggled successfully", doctor: out });
     }


}catch (err) {
  console.log("toggleDoctorAvailability error:", err);
  return res.status(500).json({ success: false, message: "Server error" });
}
}

//to login doctor and generate token
