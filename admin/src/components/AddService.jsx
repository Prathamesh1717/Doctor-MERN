...
const API_BASE = "https://doctor-mern-backend-l6gr.onrender.com";
 const fileRef = useRef(null);
  ...
  const [hasExistingImage, setHasExistingImage] = useState(false); //to update image
  const [removeImage, setRemoveImage] = useState(false); //existing one can be removed
  ...
  //prevent user select previous year/month/date.from today
  useEffect(() => {
...
//image change
  function handleImageChange(e) {
    ...
    //remove ecisting image if user chose new
    setRemoveImage(false);
    setHasExistingImage(false);
  }
