...
//take slot with date time return date obj
...
const AppointmentsPage = () => {
  ...
  const isAdmin = true; //admin logged in

  const [appointments, setAppointments] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const [query, setQuery] = useState(""); //use for filtration
  const [filterDate, setFilterDate] = useState(""); //srt date
 ...
 useEffect(() => {
  ...
  //if admin want to mark as completed
  async function adminCompleteAppointment(id) {
    const appt = appointments.find((x) => x.id === id);
    if (!appt) return;
    ...
    //dont allow if already cancelled or completed
    if (isCancelled || isCompleted) return;

    const ok = window.confirm(
      `As admin, mark appointment for ${appt.patientName} with ${
        appt.doctorName
      } on ${formatDateISO(appt.slot.date)} at ${appt.slot.time} as COMPLETED?`
    );
    if (!ok) return;
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body?.message || `Failed to complete appointment (${res.status})`);
      }
      const data = await res.json();
      const updated = data?.appointment || data?.appointments || null;
      if (updated) {
...
{loading ? (
    ...
    <div className= {pageStyles.noResultsContainer}>No appointments found.
    </div>
):(
                  ...
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      {isAdmin && (
                        <>
                          <button
                            onClick={() => adminCompleteAppointment(a.id)}
                            title={
                              isDisabled
                                ? isCancelled
