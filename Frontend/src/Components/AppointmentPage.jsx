...
const AppointmentPage = () => {
  ...
  //to load service from server side
  ...
  }, [searchParams, loadDoctorAppointments]);
  ...
  const appointmentData = useMemo(() => {
    return doctorAppts
      .map((a) => {
        ...
        // Only override if status is still Pending: if online payment is paid, should be Confirmed
        if (status === "Pending" && payment === "Online" && a.payment && a.payment.status === "Paid") {
          status = "Confirmed";
        }
  ...
  const serviceData = useMemo(() => {
    return serviceAppts
      .map((s) => {
        ...
        // Only override if status is still Pending: if payment is paid, should be Confirmed
        if (status === "Pending" && s.payment && s.payment.status === "Paid") {
          status = "Confirmed";
        }
...
Your Doctor Appointments
            ...
            {error && (
                <div className = {appointmentPageStyles.emptyStateText} style={{ color: 'red' }}>
                    Error: {error}
                </div>
            )}
            {!loadingDoctors && !error && appointmentData.length === 0 && (
...
<CalendarDays className = {iconSize.medium} /> {item.date}
                        ...
                        <p className = {cardStyles.timeContainer}>
                            ...
                            <div className = {cardStyles.badgesContainer}>
                                ...
                                {item.status === "Reschedule" && item.rescheduledTo ? (
